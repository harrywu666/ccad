...
def _extract_pct(item: Dict[str, Any], key: str, evidence: Dict[str, Any]) -> Dict[str, float] | None:
    """Extract global_pct dict from item or evidence, with fallback to legacy grid field."""
    raw = item.get(key) or evidence.get(key)
...
def parse_dimension_pair_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    source_pct = _extract_pct(item, "source_pct", evidence)
    target_pct = _extract_pct(item, "target_pct", evidence)
    # Backward compatibility: also check legacy grid fields
    ...
    if not location:
        if source_pct or target_pct:
            s = f"x:{source_pct['x']:.0f}%,y:{source_pct['y']:.0f}%" if source_pct else "?"
            t = f"x:{target_pct['x']:.0f}%,y:{target_pct['y']:.0f}%" if target_pct else "?"
            location = f"{s} -> {t}"
        elif source_grid or target_grid:
