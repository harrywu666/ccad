...
def test_index_agent_report_is_written_as_internal_event_and_runner_broadcast(monkeypatch, tmp_path):
    session_local, models, state_transitions, reports = _load_runtime(monkeypatch, tmp_path)

    db = session_local()
    try:
        db.add(models.Project(id="proj-index-report-bridge", name="Index Report Bridge"))
        db.add(models.AuditRun(project_id="proj-index-report-bridge", audit_version=1, status="running"))
        db.commit()
    finally:
        db.close()

    state_transitions.append_agent_status_report(
        "proj-index-report-bridge",
        1,
        step_key="index",
        agent_key="index_review_agent",
        agent_name="索引审查Agent",
        progress_hint=24,
        report=reports.IndexAgentReport(
            batch_summary="索引复核结果不稳",
            blocking_issues=[{"kind": "unstable_output", "stage": "index_visual_review"}],
    ...
    try:
        rows = (
            ...
            .filter(
                models.AuditRunEvent.project_id == "proj-index-report-bridge",
    ...
    assert "索引审查Agent" in broadcast_row.message
