...
export default function SettingsFeedbackAgentPrompts() {
  ...
  const loadItems = async () => {
    try {
      ...
      setError(getErrorMessage(value, '读取误报反馈 Agent 提示词失败。'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (key?: EditableAsset['key']) => {
    ...
    try {
      ...
      setSaveMessage(key
        ...
        : '误报反馈 Agent 提示词已保存，后面的误报反馈会直接用新内容。');
    } catch (value) {
      setError(getErrorMessage(value, '保存误报反馈 Agent 提示词失败。'));
    } finally {
      setSaving(false);
      setSavingKey(null);
    }
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="space-y-3">
            ...
            <CardDescription className="max-w-[980px] text-[14px] leading-7 text-muted-foreground">
              这里单独管理误报反馈 Agent 的三份核心文本：用户任务模板、AGENT.md、SOUL.md。你改完保存，后面新发起的误报反馈线程会直接用这版。
            </CardDescription>
            <div className="border-l-2 border-primary pl-3 text-[13px] leading-6 text-foreground">
              这部分不走通用审图 stage 配置，而是直接对应误报反馈 Agent 运行时实际读取的文件。
            </div>
          </div>
          <Button
            type="button"
