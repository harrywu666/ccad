...
export function useIssueFocus({
  ...
  useEffect(() => {
    ...
    if (focusAnchorStatus === 'pdf_low_confidence' || focusAnchorStatus === 'pdf_visual_mismatch') return;
    ...
    if (!hasRegion && !hasPoint) return;
    if (!hasRegion && typeof focusAnchor?.confidence === 'number' && focusAnchor.confidence < 0.6) return;
    ...
    if (hasRegion && region) {
      ...
      const paddedWidth = Math.max(boxWidth * 3.0, 180);
      const paddedHeight = Math.max(boxHeight * 3.0, 180);
      focusScale = Math.min(
        2.6,
        Math.max(
          1.15,
      ...
      focusScale = Math.min(
        ...
        Math.max(
          1.2,
          Math.max(
            stageSize.width / Math.max(imageAsset.width * 0.32, 1),
            stageSize.height / Math.max(imageAsset.height * 0.32, 1),
          ),
        ),
      );
